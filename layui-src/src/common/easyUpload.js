...
(function ($) {
  $.fn.easyUpload = function (opts) {
    var defaults = {
      allowFileTypes: '*.pdf;*.doc;*.docx;*.jpg;*.docx;*.png',//允许上传文件类型，格式'*.pdf;*.doc;'
      allowFileSize: 100000,
      selectText: '选择文件',//上传按钮文案
      multi: true,//是否允许多文件上传
      multiNum: 5,//多文件上传时允许的有效文件数
      showNote: true,//是否展示文件上传说明
      note: '提示：最多上传5个文件，超出默认前五个，支持格式为：doc、docx、pdf、png、docx',//文件上传说明
      showPreview: true,
    ...
    this.each(function (index, element) {
      ...
      var easyManager = {
        init: function () {
          ...
          $html += '<span style="background:#38f;color:white" class="easy_upload_select noselect">' + option.selectText + '</span>';
          $html += '<span  style="background:#38f;color:white" class="easy_upload_head_btn1 noselect">全部上传</span>';
          $html += '<i class="easyUploadIcon noselect head_check" data-checked="no" style="display:none">&#xe693;</i>';
        ...
        bindHead: function () {
          ...
          $('.easy_upload_select').off('click').click(function () {
            console.log('..easy_upload_select')
            $(this).parent().find('.fileInput').trigger('click');
          });
          $('.fileInput').off('change').change(function (event) {
            console.log('.fileInput',this)
            console.log('.fileInput event',event.target.files[0])
            ...
            console.log('_checkFile',fileArr)
          ...
          $('.easy_upload_head_btn1').off('click').click(function(){
        	  ...
        	  if(!status.attr('id')){
        	  $('.easy_upload_upbtn').each(function(i,item){
        		 item.click(); 
        		 status.attr('id',new Date().getDate())
        	  })}
        	/*alert("上传")
        	$('.head_check').click()
        ...
        _renderFile: function (fileArr, target) {
          ...
          function render(file) {
            ...
            $html += file.allow ? '<p  style="background:#38f;color:white" class="easy_upload_upbtn btn noselect">上传</p>' : '';
            $html += '<p  style="background:#38f;color:white" class="easy_upload_delbtn btn noselect">删除</p>';
            $html += '</div>';
            $html += '<div class="easy_upload_checkone queue_item-section">';
            $html += '<i style="display:none" class="easyUploadIcon noselect queue_check queue_check_allow_'+ file.allow +'" data-checked="no" data-up="1">&#xe693;</i>';
            $html += '</div>';
            if (option.multi) {
              $(queueUl).append($html);
            } else {
          ...
          console.log('_renderFile',fileArr, target)
          for (var i = 0; i < fileArr.length; i++) {
            if (option.multi) {
        ...
        _uploadFile: function(target) {
          ...
          function upload() {
            if (allowNewPost) {
              ...
              $.ajax({
                url: option.url+'&folderId='+folderId,
                type: "POST",
                data: fd,
                dataType:'json',
                processData: false,
                ...
                success: function (res) {
                  ...
                  if(frameElement.api){
                	  frameElement.api.opener.uploadSuccess('',res);
                  }else{
                	  parent.uploadSuccess(res);
                  }
                },
                error: function (res) {
        ...
        _deleFiles: function(arr,target) {
          ...
          arr.forEach(function(item){
            $(target).find('.easy_upload_queue_item[data-index='+ index +']').hide().find('.queue_check').hide();
