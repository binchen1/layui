...
(function (window, undefined) {
    ...
    var dfoption = {
        ...
        //样品ID
        testSampleId: '',
        // 未知属性 by chenlm at 2019-06-27 17:11:53
        //velName: '',
        //样品名称
        testSampleName: '',
        ...
        // 按照参数计算费用时使用 by chenlm at 2019-06-27 17:09:12
        testObjectPrice: 0,
    ...
    $.extend(testobject, {
        initFromSample: function (sampleinfo, completed) {
            ...
            this.testSampleId = sampleinfo.THE_TEST_SAMPLE_ID;
            ...
            this.THE_TEST_SAMPLE_ID = sampleinfo.THE_TEST_SAMPLE_ID;
        ...
        initFromSession: function (loadedData) {
            ...
            if (!objectString) { // 编辑或查看详情
                console.log("xxx init from session : " + objectString);
                if (loadedData) {
                    loadedData(false);
                }
                return;
            ...
            //加载样品ID
            this.THE_TEST_SAMPLE_ID = sobj.testSampleId;
            ...
            console.log('load from session done')
        },
        ...
        init_BuidTestParamUI: function (tableID) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                for (var j = 0; j < params.length; j++) {
                    ...
                    // this.testObjectPrice = sumAmount + row.sharePrice;
                }
            ...
            $('.group-fees').each(function(){
                console.log('this',$(this).html())
                that.testObjectPrice += parseFloat($(this).html())
            })
        },
        /**
         *构建计数数量微调器
        ...
        init_BuildTestOTherInfo: function () {
            this._loadTestOtherInfo(function () {
                ...
                for (let i = 0; i < testobject.WaitTestOtherInfoList.length; i++) {
                    ...
                    if (element.parentID==testobject.THE_TEST_SAMPLE_ID || itemIds.indexOf(element.parentID) >= 0) {
                        //从之前的数据里  获取默认值
                        //根据ID获取上次数据
                        ...
                        // add start by weiheng 20190629 - 这里数据不对，传错值了，我改下了
                        var newEle = JSON.parse(JSON.stringify(element));
                        newEle.attributeId = element.id;
                        delete newEle.id;
                        delete newEle.inputHistory;
                        // add end by weiheng 20190629
        ...
        _loadTestParamByBCSampleID: function (ParamLoaded) {
            ajaxSubmitResponseJSON({
                ...
                data: {
                    "sampleId": this.THE_TEST_SAMPLE_ID,
        ...
        _loadTestOtherInfo: function (testtherinfoLoaded) {
            ...
            var sampleID = this.THE_TEST_SAMPLE_ID;
            var itemIds = [];
    ...
    //TestOtherInfo相关
    $.extend(testobject, {
        LoadDataFromUI_SamplingInfo:function(){

            this.samplingPlace = $('#samplingPlace').val();
            this.samplingDate = $('#samplingDate').val();
            this.samplingCompany = $('#samplingCompany').val();
            this.samplePerson = $('#samplePerson').val();
            this.samplePersonNumber = $('#samplePersonNumber').val();
        },
        ...
        RefMaterial_Add: function (adata) {
            ...
            for (let index = 0; index < data.length; index++) {
                ...
                if (element.refer == "1") {
                    ...
                    // this.parentSampleId = element.testObjectId;
                    if (hasAdded) {
                        continue;
                    }
                }
    ...
    $.extend(testobject, {
        ...
        LoadDataFromUI_TestParams: function () {
            ...
            for (var i = 0; i < params.length; i++) {
                ...
                // 修复test库无此属性问题 by chenlm at 2019-07-03 11:26:11
                testParamO.count = curParam.count;
        ...
        LoadDataFromUI_Periods: function () {
            ...
            if (rows && rows.length > 0) {
                for (var i = 0; i < rows.length; i++) {
                    var period = {};
                    var row = $(rows[i]);
                    period.periodCode = row.find('input.periodCode').val();
                    period.formingDate = row.find('input.formingDate').val();
