...
function doNewCharge() {
  ...
  goNewCharge('新增收费', 'chargeController.do?goAddFee', 'feeModelList', '60%', '80%');
}
...
function goRollbackPage(){
    ...
    if(selectItems.length === 0){
        layer.msg('请勾选一条委托费用', {icon: 5});
    } else if(selectItems.length === 1){
      showRollbackPage('fee',selectItems[0].id);
    }else{
      layer.msg('一次只能回退一条记录', {icon: 5});
    }
    console.info("selectItems", selectItems);
}