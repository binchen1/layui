  ...
  if (testObjects) {
    ...
    for (var i = 0; i < objectAry.length; i++) {
      if (objectAry[i].parentSampleId == null) { //主试验样品
        applyAddOrUpdateTestObject(objectAry[i],true); //展示
        parentTestObject = objectAry[i];
      } else {
        applyAddOrUpdateTestObject(objectAry[i],false, parentTestObject);
      }
    }
  }
}
 ...
 */
function applyAddOrUpdateTestObject(testObject,isParent, parentTestObject) {
  ...
  if(isParent){
    ...
    htm += '<td class="value" style="width:40px;"></td>';
  }
  ...
  if(isParent){
    ...
    htm += '<td billingItem="1" class="value">' + 0 + '</td>';
  }
  htm += '<td class="value">';
  ...
  var index = layer.open({
    ...
    yes: function (index, layero) {
      ...
      for (var i = 0; i < testObjects.length; i++) {
        ...
        applyAddOrUpdateTestObject(testObjects[i],true);
        ...
        if (!!mater && mater.length > 0) {
          for (var j = 0; j < mater.length; j++) {
            applyAddOrUpdateTestObject(mater[j],false, testObjects[i]);
          }
        }
      }
